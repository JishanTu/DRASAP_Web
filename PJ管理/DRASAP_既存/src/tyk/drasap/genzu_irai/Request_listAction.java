 ...
 * 原図庫作業依頼リストを表示する。
 */
public class Request_listAction extends Action {
	private static DataSource ds;
	// --------------------------------------------------------- Instance Variables
	// --------------------------------------------------------- Methods
	private static Category category = Category.getInstance(Request_listAction.class.getName());
	...
	public ActionForward execute(
		...
		Request_listForm request_listForm = (Request_listForm) form;
		...
		if("".equals(request_listForm.action) || request_listForm.action == null){
			...
			try{
				...
				//原図庫作業依頼リストに表示する
				Request_listElement[] items = (Request_listElement[])request_listForm.iraiList.toArray(new Request_listElement[0]);
				int countKey = 0;
...
//					String busyo_name = items[i].getBusyo_name();//部署名
					...
					if("図面登録依頼".equals(irai) || "図面出力指示".equals(irai)){
						...
						if(countKey == 0 || id.equals(items[i-1].getJob_id()) && rowNo.equals(items[i-1].getRowNo())){
							if(touroku == null || "0".equals(touroku)){
								...
								totalCount++;
								if(items.length-1 != i){
									if(!id.equals(items[i+1].getJob_id()) || !rowNo.equals(items[i+1].getRowNo())){
										flag = false;
									}
								}else{
									flag = false;
		...
		}else if("button_print".equals(request_listForm.action)){
			category.debug("print -->");
			return(mapping.findForward("print"));

		}else if("button_history".equals(request_listForm.action)){
			// 作業依頼履歴ボタンが押されていたら、
	...
	public java.util.ArrayList<Request_listElement> getData_Search(Request_listForm request_listForm, ArrayList<Request_listElement> iraiList, User user, String classId) throws Exception {
...
//				String sagyou = rs.getString("JOB_STAT");//作業ステータス
		...
		Request_listElement[] items = (Request_listElement[])irai_List.toArray(new Request_listElement[0]);
		for(int i = 0; i < items.length; i++){
			String seq = items[i].getSeq();//シーケンス番号
...
//				String messageNo = rs.getString("MESSAGE_NO");//メッセージ番号
	 ...
	 */
	public void doUpdate(java.sql.Connection conn, Request_listForm request_listForm, User user, String seq, String touroku, String message, String id, String rowNo)throws Exception {
	 ...
	 */
	public void doUpdate_irai(java.sql.Connection conn, Request_listForm request_listForm, User user, String touroku, String message, String id, String rowNo)throws Exception {
	 ...
	 * 変更['04.Jul.14]・・・作業者ID、作業者名を更新する。
	 */
	public void doUpdate_status(java.sql.Connection conn, Request_listForm request_listForm, User user, String id, String rowNo, String status)throws Exception {
