 ...
 */
public class Management_LoginAction extends Action {
	private static Category category = Category.getInstance(Management_LoginAction.class.getName());
	// --------------------------------------------------------- Methods
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,
		...
		Management_LoginForm loginForm = (Management_LoginForm) form;
...
//		String passwdFile = beaHome + DrasapPropertiesFactory.getDrasapProperties(this).getProperty("tyk.csvdef.delHostReqPs.path");
		...
		String passwdFile = apServerHome + DrasapPropertiesFactory.getDrasapProperties(this).getProperty("tyk.csvdef.delHostReqPs.path");
// 2013.06/14 yamagishi modified. end
		try {
			delHostReqPs = new CsvItemStrList(passwdFile);
			if (delHostReqPs.searchLineData(user.getId()) != null) {
				if (!delHostReqPs.searchLineData(user.getId()).get(1).equals(passwd)) {
					errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("search.delHost.passMissmatch","Password"));
				}
			} else {
			...
			// for ユーザー
			errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("search.delHost.FileNotFound",passwdFile));
			// for システム管理者
