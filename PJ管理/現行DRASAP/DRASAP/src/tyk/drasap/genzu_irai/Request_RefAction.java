 ...
 * 原図庫作業依頼詳細
 */
public class Request_RefAction extends Action {
	private static DataSource ds;
	// --------------------------------------------------------- Instance Variables
	// --------------------------------------------------------- Methods
	private static Category category = Category.getInstance(Request_RefAction.class.getName());
	...
	public ActionForward execute(
		...
		Request_RefForm request_refForm = (Request_RefForm) form;
...
//		ActionErrors errors = new ActionErrors();
		...
		if("".equals(request_refForm.action) || request_refForm.action == null){
			...
			Request_RefElement[] items = (Request_RefElement[])request_refForm.irai_List.toArray(new Request_RefElement[0]);
	...
	public void doSyokika(User user, Request_RefForm request_refForm, java.sql.Connection conn) throws Exception{
		...
		ArrayList irai_List = new ArrayList();//依頼中の内容を格納する
 ...
 */
	public void doTenkai(User user, Request_RefForm request_refForm, String job_id, String rowNo) throws Exception{
		ArrayList irai_List = new ArrayList();
		ArrayList iraiList = new ArrayList();
		java.sql.Connection conn = null;
		java.sql.Statement stmt = null;//データー存在チェック用コネクションステートメント
		java.sql.ResultSet rs = null;
		try{
			...
			Request_RefElement[] item = (Request_RefElement[])irai_List.toArray(new Request_RefElement[0]);
