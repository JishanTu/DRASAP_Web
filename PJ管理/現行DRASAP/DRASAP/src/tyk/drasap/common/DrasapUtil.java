...
public class DrasapUtil {
	...
	private static int convertDrwgSizeToInt(String size){
		String sizeInt = (String) drwgSizeMap.get(size);
		if(sizeInt == null){
	...
	public static String decidePrintSizeForRequest(String drwgSize, String specifiedSize, Printer printer){
		if("ORG".equals(specifiedSize)){
			...
			} else if("A0L".equals(drwgSize) && printer.isPrintableA0()){
				// 図面サイズ=A0Lで、A0が印刷可能なら、A0を返す
				return "A0";
			} else {
				return null;// 出力サイズ指定エラー
			}
		} else if(specifiedSize.endsWith("%")){
			// サイズが'%'で終わっている場合、////////////////////////////////////////////
	...
	public static String convertDrwgSizeToXX(String ratio, String drwgSize){
		if("70.7%".equals(ratio)){
			if("A0L".equals(drwgSize)){
				return "A1L";
			} else if("A0".equals(drwgSize)){
				return "A1";
			} else if("A1L".equals(drwgSize)){
				return "A2L";
			} else if("A1".equals(drwgSize)){
				return "A2";
			} else if("A2L".equals(drwgSize)){
				return "A3L";
			} else if("A2".equals(drwgSize)){
				return "A3";
			} else if("A3".equals(drwgSize)){
				return "A4";
			} else {
				throw new IllegalArgumentException("指定した図面サイズを70.7%にすることはできません。{" + drwgSize + "}");
			}
		} else if("50%".equals(ratio)){
			if("A0L".equals(drwgSize)){
				return "A2L";
			} else if("A0".equals(drwgSize)){
				return "A2";
	...
	public static boolean isDigit(String src){
		for (int i = 0; i < src.toCharArray().length; i++) {
			if (i == 0 && src.charAt(i) == '-') continue;
			if (src.charAt(i) == '.') continue;
			if (!Character.isDigit(src.charAt(i))) return false;
		}
		return true;
	}
	// --------------------------------------------------------------- for Test
