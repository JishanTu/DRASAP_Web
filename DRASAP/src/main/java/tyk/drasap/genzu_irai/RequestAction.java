...
public class RequestAction extends BaseAction {
	...
	public String execute(
		...
		RequestForm requestForm = form;
		category.debug("RequestAction スタート");
		//Model errors = messageSource.getMessages();//エラー内容を表示する
		...
		if ("button_irai".equals(requestForm.action)) { //原図庫作業依頼をする
			if ("".equals(requestForm.irai)) {
				MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.irai.required", null, null));//依頼区分を選択してください
				//saveErrors(request, errors);
				request.setAttribute("errors", errors);
				return "success";
			}
			//データのチェック
			CheckData(requestForm, errors, request);
				...
				try {
					...
					if ("on".equals(requestForm.hiddenNo1)) { //1行目の内容を依頼追加する
						...
						if ("A".equals(irai) || "B".equals(irai)) {//図面登録依頼、図面出力指示の場合は図面を展開することができる
							String gyoNo = "1";//行
							doSitei(requestForm, errors, user, request, kaisiNo, syuryoNo, gyoNo, job_Id, conn);
					...
					if (Objects.isNull(errors.getAttribute("message"))) {
						if ("on".equals(requestForm.hiddenNo1) || "on".equals(requestForm.hiddenNo2) || "on".equals(requestForm.hiddenNo3) || "on".equals(requestForm.hiddenNo4) || "on".equals(requestForm.hiddenNo5)) {
							...
							MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("touroku.irai", null, null));
							//saveErrors(request, errors);
							request.setAttribute("errors", errors);
						}
					} else if (conn != null) {
					...
					MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.outer.exception", new Object[] { e.getMessage() }, null));
					//saveErrors(request, errors);
					request.setAttribute("errors", errors);
	 ...
	 * @param errors
	 */
	private void setFormData(RequestForm requestForm, User user, Model errors) {
		//依頼内容をセットする
		requestForm.iraiList = new ArrayList<String>();//依頼内容
		...
		//出力先をセットする
		requestForm.list = new ArrayList<RequestElement>();
		for (int i = 0; i < user.getEnablePrinters().size(); i++) {
			Printer e = user.getEnablePrinters().get(i);
			if (e.isDisplay()) {
		...
		//原図内容をセットする
		requestForm.genzuNameList = new ArrayList<String>();
		...
		//縮小のコンボ
		requestForm.syukusyouList = new ArrayList<String>();
		requestForm.syukusyouList.add("0");
		requestForm.syukusyouList.add("1");

		//サイズのコンボ
		requestForm.saizuList = new ArrayList<String>();
	 ...
	 *
	 */
	private void doSitei(RequestForm requestForm, Model errors, User user, HttpServletRequest request, String kaisiNo, String syuryoNo, String gyoNo, String job_Id, Connection conn) throws Exception {
		...
		if ("".equals(kaisiNo) || "".equals(syuryoNo)) {
			...
			if (kai_syuNo.length() == 11 || kai_syuNo.length() == 12) {
				...
				String touroku = "";//登録有無
				ArrayList<RequestResultElement> zubanList = new ArrayList<RequestResultElement>();
				...
				try {
					...
					MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage(classId, new Object[] { e.getMessage() }, null));
				...
				RequestResultElement[] items = zubanList.toArray(new RequestResultElement[0]);
				String seq = "";//シーケンス番号
				int r = 0;
				...
				MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hani_leng_l.check", null, null));
				//saveErrors(request, errors);
				request.setAttribute("errors", errors);
			}
			...
			if (Objects.isNull(errors.getAttribute("message"))) {
				if (kaisiNo.substring(0, 10).equals(syuryoNo.substring(0, 10))) {
					...
					if (kai < syu && !"?".equals(kaisiNo.substring(10, 11)) && !"?".equals(syuryoNo.substring(10, 11))) {
						...
						String touroku = "";//登録有無
						ArrayList<RequestResultElement> zubanList = new ArrayList<RequestResultElement>();//図番があれば格納する
						try {
							stmt = conn.createStatement();
						...
						MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hanisitei_s.required", null, null));//必ず開始番号 < 終了番号で指定してください
						//saveErrors(request, errors);
						request.setAttribute("errors", errors);
					}
					...
					if (kai < syu && !"??".equals(kaisiNo.substring(9, 11)) && !"??".equals(syuryoNo.substring(9, 11))) {
						...
						for (int t = 0; t < oiban.length; t++) {
							...
							} else if (t != oiban.length - 1 || "".equals(hani_e)) {
								//終了番号の末尾が?
								hani = hani.substring(0, 9) + oiban1;
							} else {
								//終了番号が指定
					...
					MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hanisitei.required", null, null));//図番の先頭9桁が同じでない場合は範囲指定はできない
					//saveErrors(request, errors);
					request.setAttribute("errors", errors);
				}
			}
	...
	private void doHani_s(RequestForm requestForm, Model errors, Connection conn, String hani, User user) throws Exception {
	...
	private void doHani_e(RequestForm requestForm, Model errors, Connection conn, String hani, User user) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void doInsert1(RequestForm requestForm, Model errors, User user, String irai, String zumenAll,
		...
		try {
			...
			ErrorLoger.error(user, this,DrasapPropertiesFactory.getDrasapProperties(this).getProperty("err.sql"));
			MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage(classId, e.getMessage()));
	 ...
	 * @throws Exception
	 */
	private void doInsert2(RequestForm requestForm, Model errors, User user, String irai, String zumenAll, String kaisiNo, String syuryoNo, String job_Id, String day, Connection conn) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void doInsert3(RequestForm requestForm, Model errors, User user, String irai, String zumenAll, String kaisiNo, String syuryoNo, String job_Id, String day, Connection conn) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void doInsert4(RequestForm requestForm, Model errors, User user, String irai, String zumenAll, String kaisiNo, String syuryoNo, String job_Id, String day, Connection conn) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void doInsert5(RequestForm requestForm, Model errors, User user, String irai, String zumenAll, String kaisiNo, String syuryoNo, String job_Id, String day, Connection conn) throws Exception {
	 ...
	 * 原図庫作業依頼展開テーブルに追加をする
	 */
	private void doInsert(Connection conn, String seq, String job_Id, String gyoNo, String drwg_no, String tourokuNo, User user, Model errors) throws Exception {
		java.sql.Statement stmt = null;//データー存在チェック用コネクションステートメント
		try {
			stmt = conn.createStatement();
	 ...
	 * 登録のない図番を原図庫作業依頼展開テーブルに追加する
	*/
	private void doNew_Insert(Connection conn, String seq, String job_Id, String gyoNo, String hani, String hanisitei, User user, Model errors) throws Exception {
		java.sql.Statement stmt = null;//データー存在チェック用コネクションステートメント
		try {
			stmt = conn.createStatement();
			String strSql = "insert into JOB_REQUEST_EXPAND_TABLE(SEQ_NO,JOB_ID,ROW_NO,EXPAND_DRWG_NO,EXIST)" +
	 ...
	 * データ登録のチェックをする
	 */
	private Model CheckData(RequestForm requestForm, Model errors, HttpServletRequest request) {
		HashSet<String> usedErrMsgSet = new HashSet<String>();// 同じエラーを複数行表示させないため
		...
		if (StringUtils.isEmpty(requestForm.hiddenNo1)
			...
			MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.zuban.required", null, null));
		}
	 ...
	 */
	private Model checkData(RequestForm requestForm, Model errors, String kaisiNo) {
		if ("".equals(requestForm.errlog)) {
			...
			if ("図面登録依頼".equals(requestForm.irai) || "図面出力指示".equals(requestForm.irai)) {
				if (kaisiNo.length() < 11 || kaisiNo.length() > 12) {
					MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.allNo.check", null, null));
	 ...
	 */
	private Model checkNumber(RequestForm requestForm, Model errors, String kaisiNo, String syuNo) {
		if (kaisiNo.length() != 11 && "".equals(requestForm.hani_t) || syuNo.length() != 11 && "".equals(requestForm.hani_t)) {
			category.debug("範囲指定は11桁のみです");
			MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hani_leng_e.check", null, null));
		} else // 開始、終了とも11ケタのときのみチェックする。
		...
		if ("".equals(requestForm.errNumber)) {
			...
			if (kaisiNo.substring(0, 10).equals(syuNo.substring(0, 10))) {
				...
				if (kai > syu && "".equals(requestForm.errNumber)) {
					MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hanisitei_s.required", null, null));
		...
		if ("原図借用依頼".equals(requestForm.irai) || "図面以外焼付依頼".equals(requestForm.irai)) {
			if ("".equals(requestForm.hani_sitei)) {
				...
				MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hani_igai.required", null, null));
	 ...
	 * @param suryoFlag
	 */
	private void checkLineData(RequestFormLineData lineData, RequestForm requestForm, Model errors,
			HashSet<String> usedErrMsgSet, RequestDataChecker checker) {
		//開始,終了番号のチェック
		...
		if(hasProhibitedChar(lineData.getKaisiNo()) || hasProhibitedChar(lineData.getSyuuryouNo())){
			if(! usedErrMsgSet.contains("error.hani_sitei.check")){
				...
				MessageSourceUtil.addAttribute(errors, "message", messageSource.getMessage("error.hani_sitei.check",null,null));
