...
public class UserKeyColDB {
	...
	public UserKeyColDB(User user, Connection conn)	throws Exception {
		...
		try{
			// ユーザーグループCDで検索する
			StringBuffer sbSql1 = new StringBuffer("select col.TABLE_NAME TABLE_NAME, col.COLUMN_NAME COLUMN_NAME ");
	...
	public UserKeyColInfo getKeyCol(String tableName) {
	    for (int i = 0; i < keyColList.size(); i++) {
		    UserKeyColInfo userKeyColInfo = (UserKeyColInfo)keyColList.get(i);
