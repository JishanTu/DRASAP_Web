...
public class TableInfoDB {
	...
	public static TableInfo getTableInfoArray(String table_name, Connection conn)
		...
		try {
			// ユーザーグループCDで検索する
			StringBuilder sbSql1 = new StringBuilder("select column_name, data_type, data_length, nullable from user_tab_cols ");
			...
			//
			while (rs1.next()) {
				colInfos.add(new TableColInfo(rs1.getString("column_name"), rs1.getString("data_type"), rs1.getString("data_length"), rs1.getString("nullable")));
			}
