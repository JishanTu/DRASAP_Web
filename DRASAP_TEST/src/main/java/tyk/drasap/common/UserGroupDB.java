...
public class UserGroupDB {
	...
	public static ArrayList<UserGroup> getUserGroupArray(ArrayList<String> userGroupCodeArray, Connection conn)
		...
		try {
			// ユーザーグループCDで検索する
			StringBuilder sbSql1 = new StringBuilder("select * from USER_GROUP_MASTER");
			sbSql1.append(" where USER_GRP_CODE in (");
			for (int i = 0; i < userGroupCodeArray.size(); i++) {
				sbSql1.append("'");
				sbSql1.append(userGroupCodeArray.get(i));
				sbSql1.append("',");
			}
			// 最後のカンマが余分なので削除
			...
			while (rs1.next()) {
				...
				if (rs1.getString("PRINTER_ID01") != null) {
					printerId.add(rs1.getString("PRINTER_ID01"));
				}
				if (rs1.getString("PRINTER_ID02") != null) {
					printerId.add(rs1.getString("PRINTER_ID02"));
				}
				if (rs1.getString("PRINTER_ID03") != null) {
					printerId.add(rs1.getString("PRINTER_ID03"));
				}
				if (rs1.getString("PRINTER_ID04") != null) {
					printerId.add(rs1.getString("PRINTER_ID04"));
				}
				if (rs1.getString("PRINTER_ID05") != null) {
					printerId.add(rs1.getString("PRINTER_ID05"));
				}
				if (rs1.getString("PRINTER_ID06") != null) {
					printerId.add(rs1.getString("PRINTER_ID06"));
				}
				if (rs1.getString("PRINTER_ID07") != null) {
					printerId.add(rs1.getString("PRINTER_ID07"));
				}
				if (rs1.getString("PRINTER_ID08") != null) {
					printerId.add(rs1.getString("PRINTER_ID08"));
				}
				if (rs1.getString("PRINTER_ID09") != null) {
					printerId.add(rs1.getString("PRINTER_ID09"));
				}
				if (rs1.getString("PRINTER_ID10") != null) {
					printerId.add(rs1.getString("PRINTER_ID10"));
				}
