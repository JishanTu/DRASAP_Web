...
public class RequestHistoryElement implements Comparable<Object> {
	...
	public RequestHistoryElement(String newCompleteDate, String newCompleteUser,
		...
		completeDate = newCompleteDate;// 作業完了日時
		if (completeDate == null) {
			completeDate = "";
		}
		completeUser = newCompleteUser;// 作業完了者名
		if (completeUser == null) {
			completeUser = "";
		}
		requestDate = newRequestDate;// 依頼日時
		jobName = newJobName;// ジョブ名
		if (jobName == null) {
			jobName = "";
		}
		drwgNo = newDrwgNo;// 図番
		goukiNo = newGoukiNo;// 号機
		if (goukiNo == null) {
			goukiNo = "";
		}
		genzuContent = newGenzuContent;// 原図内容
		...
		scaleMode = newScaleMode;// 縮小区分
		if (scaleMode == null) {
			scaleMode = "";
		}
		scaleSize = newScaleSize;// 縮小サイズ
		if (scaleSize == null) {
			scaleSize = "";
		}
		message = newMessage;// メッセージ
		if (message == null) {
			message = "";
		}
		requestUser = newRequestUser;// 依頼者名
		deptName = newDeptName;// 依頼部署
	}
